...
// Skip these tests if environment variables are not set (e.g., in CI)
const shouldSkip = !SUPABASE_URL || !ANON_KEY;

if (shouldSkip) {
  describe('bouquets RLS policies', () => {
    it('skipped - missing environment variables', () => {
      console.log(
        'Skipping RLS tests: NEXT_PUBLIC_SUPABASE_URL or NEXT_PUBLIC_SUPABASE_ANON_KEY not set',
      );
    });
  });
} else {
  ...
  beforeAll(async () => {
    ...
    if (bouquetError || !bouquetData) {
      throw new Error(
        `Failed to insert test bouquet: ${bouquetError?.message}`,
      );
    }
